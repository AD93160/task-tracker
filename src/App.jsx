            ...
            <div onDragOver={e=>{e.preventDefault();setDropZone("today");}} onDrop={onDropToday}
              ...
              {teamTasks.filter(t=>t.scheduledFor==="today"&&t.status!=="Terminé"&&(t.memberVisible!==false||t.hiddenBy===user.uid)).length===0 ? (
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:12, alignContent:"flex-start", flex:1, padding:"6px 4px 6px 4px" }}>
                  {teamTasks.filter(t=>t.scheduledFor==="today"&&t.status!=="Terminé"&&(t.memberVisible!==false||t.hiddenBy===user.uid)).map(task => {
