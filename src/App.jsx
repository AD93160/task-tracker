...
function AdBanner() {
  ...
  // Coque Capacitor (Android). Le login Google y est masqué : signInWithPopup
  // est bloqué par Google en WebView, et le plugin natif qui le remplace
  // demande une configuration Firebase (SHA-1, google-services.json) qui
  // n'est pas en place. Email/mot de passe reste pleinement fonctionnel.
...
export default function App() {
  ...
  useEffect(() => { if (user?.uid) localStorage.setItem(`tt_userPseudo_${user.uid}`, JSON.stringify(userPseudo)); }, [userPseudo, user?.uid]);
  ...
  useEffect(() => {
    ...
    // Le push web repose sur un service worker et le service push du
    // navigateur : rien de tout cela n'existe dans la WebView Capacitor.
    // Les notifications natives Android passeront par
    // @capacitor/push-notifications, non embarqué dans cette version.
  ...
  if (!user) return (
    <div style={{ height:"100vh", background:CANON.grad, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:"'DM Mono','Courier New',monospace", color:"#2a4a3a" }}>
      ...
      <div style={{ background:"#ffffffcc", backdropFilter:"blur(8px)", border:"1px solid #86EFAC66", borderRadius:16, padding:"28px 32px", width:"100%", maxWidth:320, boxShadow:"0 8px 32px #86EFAC33" }}>
        ...
        {!isNativeApp && <button onClick={loginGoogle} style={{ width:"100%", padding:"10px 12px", background:"#fff", border:"1px solid #86EFAC88", borderRadius:8, color:"#2a4a3a", fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
              <button
                ...
                onClick={() => {
                  ...
                  if (dlUrl) {
                    if (/\.(dmg|exe|AppImage)$/.test(dlUrl)) {
            ...
            <div style={{ display:"flex", gap:10, alignItems:"center" }}>
              ...
              {user && team && (
                <div style={{ display:"flex", background:theme.bg, border:`1px solid ${theme.border}`, borderRadius:8, overflow:"hidden", fontSize:10 }}>
                  <button onClick={()=>setTeamSpace(false)} style={{ padding:"5px 10px", background:!teamSpace?theme.accent:"transparent", border:"none", color:!teamSpace?"#fff":theme.textMuted, cursor:"pointer" }}>{userPseudo ? userPseudo.split(" ")[0].slice(0,10) : "Perso"}</button>
                      ...
                      {isMobile && (
                        <div style={{ paddingLeft:40,marginTop:6 }}>
                          ...
                          <div style={{ display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",borderTop:`1px solid ${theme.border}44`,paddingTop:5 }}>
                            <span style={{ fontSize:9,color:theme.textMuted+"88",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>par {(team?.members||[]).find(m=>m.uid===task.createdBy)?.displayName || task.createdByEmail?.split("@")[0] || team.adminEmail?.split("@")[0]}</span>
                            ...
                            {isAdminRole(teamRole) && <button onClick={e=>{e.stopPropagation();toggleMemberVisible(task.id,task.memberVisible);}} style={{ background:"transparent",border:`1px solid ${theme.border}`,borderRadius:5,padding:"3px 8px",fontSize:12,cursor:"pointer",opacity:task.memberVisible!==false?1:0.4 }} title={task.memberVisible!==false?"Masquer aux membres":"Rendre visible aux membres"}>{task.memberVisible!==false?"🔓":"🔒"}</button>}
